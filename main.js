...
const validateCred = (aray) => {
    let sum = 0;
    for (let i = aray.length - 1; i >= 0; i--) {
        let currValue = aray[i];
        if ((aray.length - 1 - i) % 2 === 1) {
        currValue *= 2;
        if (currValue > 9) {
            currValue -= 9;
        }
...
// find Invalid Cards
let findInvlidCards = (nestedArray) => {
    let invalidCards = [];
...
console.log(IdInvalidCardCompanies(findInvlidCards(batch)));
